...
class MockFactory(BaseFactory):
    ...
    def with_special_fields(self) -> dict[str, Any]:
        return {
            "field1": "special_value",
            "field3": "special_field3"
        }
...
class TestBaseFactory(unittest.TestCase):
    ...
    def test_build_creates_unsaved_document(self, mock_get_doc):
    ...
    @patch('frappe.get_doc')
    def test_build_with_traits(self, mock_get_doc):
    ...
    @patch('frappe.get_doc')
    def test_build_with_overrides(self, mock_get_doc):
        """Test that build() applies overrides correctly"""
        mock_get_doc.return_value = self.mock_doc
        
        result = MockFactory.build(name="Override Name", field1="override_value", new_field="new_value")
    ...
    @patch('frappe.get_doc')
    def test_build_precedence_order(self, mock_get_doc):
        """Test that overrides take precedence over traits, which take precedence over defaults"""
        mock_get_doc.return_value = self.mock_doc
        
        result = MockFactory.build("with_custom_name", "with_numbers", name="Final Override", field2=777)
    ...
    @patch('frappe.get_doc')
    def test_create_saves_document(self, mock_get_doc):
    ...
    @patch('frappe.get_doc')
    def test_create_with_traits_and_overrides(self, mock_get_doc):
    ...
    @patch('frappe.get_doc')
    def test_build_list(self, mock_get_doc):
    ...
    @patch('frappe.get_doc')
    def test_build_list_with_traits_and_overrides(self, mock_get_doc):
    ...
    @patch('frappe.get_doc')
    def test_create_list(self, mock_get_doc):
    ...
    @patch('frappe.get_doc')
    def test_create_list_with_traits_and_overrides(self, mock_get_doc):
    ...
    def test_invalid_traits_raises_error(self):
        """Test that using invalid traits raises TypeError"""
        with self.assertRaises(TypeError) as context:
            MockFactory("invalid_trait", "with_custom_name")
        
        self.assertIn("must be a subset of the set of valid traits", str(context.exception))
        self.assertIn("invalid_trait", str(context.exception))

    def test_valid_traits_detection(self):
        """Test that valid traits are correctly detected"""
        # This should not raise an error
        try:
            factory = MockFactory("with_custom_name", "with_special_fields", "with_numbers")
            valid_traits = [
                name for name, value in MockFactory.__dict__.items()
                if isinstance(value, property) and name != "default_attributes"
            ]
            expected_traits = {"with_custom_name", "with_special_fields", "with_numbers"}
            self.assertTrue(expected_traits.issubset(set(valid_traits)))
        except TypeError:
            self.fail("Valid traits should not raise TypeError")

    @patch('frappe.get_doc')
    def test_empty_default_attributes(self, mock_get_doc):
    ...
    @patch('frappe.get_doc')
    def test_doctype_cannot_be_overridden(self, mock_get_doc):
    ...
    def test_attach_del_creates_temp_subclass(self):
    ...
    def test_del_override_is_callable(self):
    ...
    @patch('frappe.get_doc')
    def test_factory_traits_attribute_is_set(self, mock_get_doc):
        """Test that factory_traits attribute is properly set"""
        mock_get_doc.return_value = self.mock_doc
        
        factory = MockFactory("with_custom_name", "with_special_fields")
        
        self.assertEqual(factory.factory_traits, ["with_custom_name", "with_special_fields"])
        self.assertIsInstance(factory.factory_traits, list)

    @patch('frappe.get_doc')
    def test_overrides_attribute_is_set(self, mock_get_doc):
    ...
    def test_attributes_property_combines_defaults_and_traits(self):
    ...
    def test_attributes_property_with_no_traits(self):
